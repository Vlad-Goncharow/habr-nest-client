...
describe('test UserNav', () => {
  ...
  it('test publications events', async () => {
    ...
    expect(subcategoryItems.some((item) => item.textContent === 'Статьи')).toBe(
      true
    )
    expect(subcategoryItems.some((item) => item.textContent === 'Посты')).toBe(
      true
    )
    expect(
      subcategoryItems.some((item) => item.textContent === 'Новости')
    ).toBe(true)
  })
  ...
  it('test favorites events', async () => {
    ...
    expect(
      subcategoryItems.some((item) => item.textContent === 'Коментарии')
    ).toBe(true)
  })
})
