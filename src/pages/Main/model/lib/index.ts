...
export const loadHabsFN = async (category:string | undefined, title: string | undefined, sort:string | undefined, order:string | undefined, page:string | undefined) => {
  if(!!category && !!title && !!sort && !!order && !!page){
    return axios.get(`/habs/search/${category}/${title}?sort=${sort}&order=${order}&page=${page}&pageSize=20`)
  }
}

export const loadAuthorsFN = async (category: string | undefined, title: string | undefined, page: string | undefined) => {
